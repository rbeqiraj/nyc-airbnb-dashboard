...
KCARD = {'background':'#EEF4FB','borderRadius':'8px','padding':'14px 10px',
         'textAlign':'center'}
        ...
        html.Div([
            ...
            html.Div([
                html.Div([
                    ...
                    dcc.Graph(id='g-bar', style={'height':'300px'}, config={'displayModeBar':False}),
                ], style={**CARD,'flex':'1','marginBottom':'0'}),

                html.Div([
                    html.Div("Room type share",
                             style={'fontSize':'12px','fontWeight':'600','color':'#555','marginBottom':'6px'}),
                    dcc.Graph(id='g-donut', style={'height':'300px'}, config={'displayModeBar':False}),
                ], style={**CARD,'flex':'1','marginBottom':'0'}),
            ...
            html.Div([
                html.Div([
                    html.Div("Review activity over time",
                             style={'fontSize':'12px','fontWeight':'600','color':'#555','marginBottom':'6px'}),
                    dcc.Graph(id='g-ts', style={'height':'260px'}, config={'displayModeBar':False}),
                ], style={**CARD,'flex':'2','marginBottom':'0'}),

                html.Div([
                    html.Div("Availability distribution",
                             style={'fontSize':'12px','fontWeight':'600','color':'#555','marginBottom':'6px'}),
                    dcc.Graph(id='g-avail', style={'height':'260px'}, config={'displayModeBar':False}),
                ], style={**CARD,'flex':'1','marginBottom':'0'}),
            ], style={'display':'flex','gap':'12px','marginBottom':'14px'}),
            ...
            html.Div([
                html.Div("Price distribution",
                         style={'fontSize':'12px','fontWeight':'600','color':'#555','marginBottom':'6px'}),
                dcc.Graph(id='g-hist', style={'height':'200px'}, config={'displayModeBar':False}),
...
def update(boroughs, rooms, price_range, avail):
    ...
    def empty(msg="No data"):
        return go.Figure().update_layout(**BL, annotations=[dict(
            text=msg, xref="paper", yref="paper", x=0.5, y=0.5,
            showarrow=False, font=dict(color="#bbb", size=12))])

    if n == 0:
        e = empty("No listings match current filters")
    ...
    g_donut = go.Figure(go.Pie(
        labels=rc['room_type'], values=rc['count'], hole=0.5,
        marker_colors=[ROOM_CLR.get(r,'#aaa') for r in rc['room_type']],
        textinfo='percent+label', textfont_size=11))
    g_donut.update_layout(**BL, showlegend=False, margin=dict(l=10,r=10,t=10,b=10))
    ...
    if len(mo) > 1:
        g_ts.add_trace(go.Scatter(
            ...
            fill='tozeroy', fillcolor='rgba(31,95,138,0.1)'))
        g_ts.add_vrect(x0='2020-03-01', x1='2021-06-01',
            fillcolor='rgba(186,117,23,0.12)', line_width=0,
            annotation_text='COVID', annotation_position='top left',
            annotation_font_size=9, annotation_font_color='#BA7517')
    g_ts.update_layout(**BL,
        xaxis=dict(showgrid=False, zeroline=False),
        yaxis=dict(showgrid=True, gridcolor='#f0f0f0', zeroline=False, title='Reviews/month'),
        showlegend=False)

    # Availability histogram
    g_avail = go.Figure(go.Histogram(
        x=dff['availability_365'], nbinsx=36,
        marker_color='#2E8B57', marker_line_color='white',
        marker_line_width=0.5, opacity=0.85))
    g_avail.update_layout(**BL,
    ...
    g_hist.update_layout(**BL,
        xaxis=dict(title='Price/night ($)', tickprefix='$', showgrid=False, zeroline=False),
        yaxis=dict(showgrid=True, gridcolor='#f0f0f0', zeroline=False),
        bargap=0.04)

    return kn,kp,ka,kr,km,fc, g_bar,g_donut,g_ts,g_avail,g_hist
